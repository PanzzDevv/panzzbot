...
const { handleCommand: handleCaseCommand } = require("../case/panzz");
const fs = require("fs");
...
async function handleSmartTriggers(m, sock, db) {
  ...
  try {
    ...
    const botName = config.bot?.name || "Panzz-AI";
    ...
    const thumbPath = "./assets/images/panzz2.jpg";
    try {
    ...
    if (
      ...
      text?.toLowerCase().includes("panzz")
    ) {
      await sock.sendMessage(
        m.chat,
        {
...
async function groupHandler(update, sock) {
  try {
    ...
    for (let participant of participants) {
      ...
      const saluranId = config.saluran?.id || "120363208449943317@newsletter";
      const saluranName =
        config.saluran?.name || config.bot?.name || "Panzz-AI";
