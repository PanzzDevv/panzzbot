...
function createContextInfo(jid, text, title = '', body = '', thumbnail = null) {
    ...
    if (jid && text) {
        ...
        contextInfo.stanzaId = 'PanzzAI' + Date.now();
    }

    if (title || body || thumbnail) {
        contextInfo.externalAdReply = {
            showAdAttribution: true,
            title: title || config.bot?.name || 'Panzz-AI',
            body: body || '',
            mediaType: 1,
...
async function serialize(sock, msg, store = {}) {
    ...
    m.botName = sock.user?.name || config.bot?.name || 'Panzz-AI';
    m.messageId = m.id;
    m.chatId = m.chat;
    m.senderId = m.sender;
    ...
    m.reply = async (text, options = {}) => {
        ...
        if (replyVariant === 2) {
            try {
                const thumbnailPath = './assets/images/panzz2.jpg';
                let thumbnail = null;
                ...
                contextInfo = {
                    ...contextInfo,
                    externalAdReply: {
                        title: config.bot?.name || 'Panzz-AI',
            ...
            try {
                ...
                contextInfo = {
                    ...
                    forwardedNewsletterMessageInfo: {
                        newsletterName: config.saluran?.name || 'Panzz-AI',
            ...
            try {
                const thumbnailPath = './assets/images/panzz.jpg';
                let thumbnail = null;
        ...
        if (replyVariant === 4) {
            ...
            const saluranName = config.saluran?.name || config.bot?.name || 'Panzz-AI';
            ...
            try {
                const thumbPath = './assets/images/panzz.jpg';
            ...
            quotedMsg = {
                ...
                message: {
                    orderMessage: {
                        orderId: '1337',
                        thumbnail: await sharp(fs.readFileSync('./assets/images/panzz2.jpg')).resize(300, 300).toBuffer() || null,
                        itemCount: `BOT READY`,
                        status: 'INQUIRY',
                        surface: 'CATALOG',
                        message: `${config.bot?.name || 'Panzz-AI'}`,
                        ...
                        contextInfo: {
                            ...
                            forwardedNewsletterMessageInfo: {
                                ...
                                newsletterName: config.saluran?.name || config.bot?.name || 'Panzz-AI',
                                serverMessageId: 127
                            }
                        }
                    }
    ...
    m.replyWithPreview = async (text, preview, options = {}) => {
        return sock.sendMessage(m.chat, {
            ...
            contextInfo: {
                externalAdReply: {
                    showAdAttribution: true,
                    title: preview.title || config.bot?.name || 'Panzz-AI',
                    body: preview.body || '',
                    mediaType: 1,
                    renderLargerThumbnail: true,
