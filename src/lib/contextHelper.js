...
const gameThumbPath = path.join(assetsPath, 'Panzz-games.jpg')
const rpgThumbPath = path.join(assetsPath, 'Panzz-rpg.jpg')
const winnerThumbPath = path.join(assetsPath, 'Panzz-winner.jpg')
...
function getGameContextInfo(title = '🎮 Panzz GAMES', body = 'Have fun playing!') {
...
function getRpgContextInfo(title = '⚔️ Panzz RPG', body = 'Adventure awaits!') {
    const saluranId = config.saluran?.id || '120363208449943317@newsletter'
    const saluranName = config.saluran?.name || config.bot?.name || 'Panzz-AI'
...
function createFakeQuoted(botName = 'Panzz-AI', verified = true) {
    return {
        key: {
            fromMe: false,
