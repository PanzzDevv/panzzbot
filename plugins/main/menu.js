...
const { generateWAMessageFromContent, proto } = require('Panzz');
/**
 * Credits & Thanks to
...
function buildMenuText(m, botConfig, db, uptime, botMode = 'md') {
    ...
    const { getCaseCount, getCasesByCategory } = require('../../case/Panzz');
...
Kamu bisa pakai aku buat cari info, ambil data, atau bantu hal-hal sederhana langsung lewat WhatsApp — praktis tanpa ribet.`
   ...
   txt += `\n\n— 🤖 *ʙᴏᴛ ɪɴꜰᴏ* 」\n`;
    ...
    txt += `\`◦\` ᴏᴡɴᴇʀ: *${botConfig.owner?.name || 'Panzz-AI'}*\n`;
    txt += `\n`;
...
function getContextInfo(botConfig, m, thumbBuffer, renderLargerThumbnail = false) {
    ...
    const saluranName = botConfig.saluran?.name || botConfig.bot?.name || 'Panzz-AI';
    ...
    const ctx = {
        ...
        externalAdReply: {
            title: botConfig.bot?.name || 'Panzz-AI',
...
async function handler(m, { sock, config: botConfig, db, uptime }) {
    ...
    const imagePath = path.join(process.cwd(), 'assets', 'images', 'Panzz.jpg');
    const thumbPath = path.join(process.cwd(), 'assets', 'images', 'Panzz2.jpg');
    const videoPath = path.join(process.cwd(), 'assets', 'video', 'Panzz.mp4');
    ...
    try {
        switch (menuVariant) {
            ...
            case 3:
                ...
                try {
                    const PanzzPath = path.join(process.cwd(), 'assets', 'images', 'Panzz.jpg');
                    if (fs.existsSync(PanzzPath)) {
                        contextThumb = fs.readFileSync(PanzzPath);
                    }
                } catch (e) {}
...
Kamu bisa pakai aku buat cari info, ambil data, atau bantu hal-hal sederhana langsung lewat WhatsApp — praktis tanpa ribet.\n\n`;
                headerText += `╭┈┈⬡「 🤖 *ʙᴏᴛ ɪɴꜰᴏ* 」\n`;
                headerText += `┃ \`◦\` ɴᴀᴍᴀ: *${botConfig.bot?.name || 'Panzz-AI'}*\n`;
                ...
                try {
                    ...
                    if (imageBuffer) {
                        try {
                            const { prepareWAMessageMedia } = require('Panzz');
                            ...
                            message: {
                                ...
                                interactiveMessage: proto.Message.InteractiveMessage.fromObject({
                                    ...
                                    footer: proto.Message.InteractiveMessage.Footer.fromObject({
                                        text: `© ${botConfig.bot?.name || 'Panzz-AI'} | ${sortedCats.length} Categories`
                                    }),
            ...
            case 6:
                const thumbPathV6 = path.join(process.cwd(), 'assets', 'images', 'Panzz3.jpg');
                const saluranIdV6 = botConfig.saluran?.id || '120363208449943317@newsletter';
                const saluranNameV6 = botConfig.saluran?.name || botConfig.bot?.name || 'Panzz-AI';
                ...
                try {
                    await sock.sendMessage(m.chat, {
                        document: imageBuffer || Buffer.from('Panzz-AI Menu'),
                ...
                try {
                    ...
                    for (const cat of sortedCatsV7) {
                        ...
                        try {
                            ...
                            const defaultV7Path = path.join(process.cwd(), 'assets', 'images', 'Panzz-v7.jpg');
                        ...
                        const cardMessage = {
                            ...
                            footer: proto.Message.InteractiveMessage.Footer.create({
                                text: `${botConfig.bot?.name || 'Panzz'} • ${cat}`
                            }),
                            nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.fromObject({
                                buttons: [{
                                    name: 'quick_reply',
                        ...
                        viewOnceMessage: {
                            message: {
                                ...
                                interactiveMessage: proto.Message.InteractiveMessage.fromObject({
                                    ...
                                    footer: proto.Message.InteractiveMessage.Footer.fromObject({
                                        text: `${botConfig.bot?.name || 'Panzz'} v${botConfig.bot?.version || '1.0'}`
                                    }),
            ...
            case 8:
                ...
                menuV8 += `╭━━〔 💡 *𝗧𝗜𝗣𝗦* 〕━━╮\n`;
                menuV8 += `│ ❸ Follow channel ${config.saluran?.link || 'Panzz-AI'}\n`;
                menuV8 += `╰━━━━━━━━━━━━━━━━━━╯\n\n`;
                ...
                const ftroliQuoted = {
                    ...
                    message: {
                        orderMessage: {
                            ...
                            status: 'INQUIRY',
                            surface: 'CATALOG',
                            message: `${botConfig.bot?.name || 'Panzz-AI'} Menu`,
                            ...
                            contextInfo: {
                                ...
                                forwardedNewsletterMessageInfo: {
                                    ...
                                    newsletterName: botConfig.saluran?.name || botConfig.bot?.name || 'Panzz-AI',
                                    serverMessageId: 127
                                }
                            }
                        }
                ...
                await sock.sendMessage(m.chat, {
                    image: fs.readFileSync('assets/images/Panzz-v8.jpg'),
                    caption: menuV8,
            ...
            case 9:
                try {
                    ...
                    const saluranNameV9 = botConfig.saluran?.name || botConfig.bot?.name || 'Panzz-AI';
                    const saluranLinkV9 = botConfig.saluran?.link || 'https://whatsapp.com/channel/0029VbB37bgBfxoAmAlsgE0t';
                    
                    const { getCasesByCategory: getCasesCatV9 } = require('../../case/Panzz');
                    ...
                    if (imageBuffer) {
                        try {
                            const resizedV9 = await sharp(fs.readFileSync('./assets/images/Panzz-v9.jpg'))
                                .resize(300, 300, { fit: 'cover' })
                                .jpeg({ quality: 80 })
                                .toBuffer();
                        ...
                        viewOnceMessage: {
                            message: {
                                ...
                                interactiveMessage: proto.Message.InteractiveMessage.fromObject({
                                    ...
                                    footer: proto.Message.InteractiveMessage.Footer.fromObject({
                                        text: `© ${botConfig.bot?.name || 'Panzz-AI'} v${botConfig.bot?.version || '1.9.0'}`
                                    }),
                                    header: proto.Message.InteractiveMessage.Header.fromObject({
                                    ...
                                    nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.fromObject({
                                        messageParamsJson: JSON.stringify({
                                            limited_time_offer: {
                                                text: botConfig.bot?.name || 'Panzz-AI',
                                                url: saluranLinkV9,
                                                copy_code: botConfig.owner?.name || 'Panzz-AI',
            ...
            case 10:
                try {
                    ...
                    const saluranNameV10 = botConfig.saluran?.name || botConfig.bot?.name || 'Panzz-AI';
                    const timeHelper = require('../../src/lib/timeHelper');
                    ...
                    const { getCasesByCategory, getCaseCount } = require('../../case/Panzz');
                    ...
                    try {
                        const imgPathV10 = path.join(process.cwd(), 'assets', 'images', 'Panzz-v9.jpg');
...
─────────────────────────
Nama    : ${botConfig.bot?.name || 'Panzz-AI'}
...
`;
                    ...
                    const buttonsV10 = [{
                        name: 'quick_reply',
                        buttonParamsJson: JSON.stringify({
                            display_text: botConfig.bot?.name || 'Panzz-AI',
                            id: `${m.prefix}allmenu`,
                    ...
                    const msgV10 = generateWAMessageFromContent(m.chat, {
                        viewOnceMessage: {
                            message: {
                                ...
                                interactiveMessage: proto.Message.InteractiveMessage.fromObject({
                                    header: proto.Message.InteractiveMessage.Header.fromObject({
                                        title: `${botConfig.bot?.name || 'Panzz-AI'} Menu`,
                                        ...
                                        productMessage: {
                                            product: {
                                                ...
                                                retailerId: botConfig.bot?.name || 'Panzz',
                                                productImageCount: 1
                                            },
                                    ...
                                    body: proto.Message.InteractiveMessage.Body.fromObject({
                                        text: `*© ${botConfig.bot?.name || 'Panzz-AI'} 2026*`
                                    }),
                                    footer: proto.Message.InteractiveMessage.Footer.fromObject({
                                        text: footerTextV10
                                    }),
        ...
        if (audioEnabled) {
            const audioPath = path.join(process.cwd(), 'assets', 'audio', 'Panzz.mp3');
