...
const pluginConfig = {
    ...
    usage: '.autoai on/off --Panzzmode=<character> --type=<text|voice>',
    example: '.autoai on --Panzzmode=furina --type=voice',
    isOwner: false,
    isPremium: false,
    isGroup: true,
    isPrivate: false,
...
async function handler(m) {
    ...
    const modeMatch = fullArgs.match(/--Panzzmode=(\w+)/i)
    ...
    if (!mode || !['on', 'off'].includes(mode)) {
        ...
        txt += `*Penggunaan:*\n`
        txt += `> .autoai on --Panzzmode=<karakter> --type=<text|voice>\n`
        txt += `> .autoai off\n\n`
        ...
        txt += `*Contoh:*\n`
        txt += `> .autoai on --Panzzmode=furina --type=text\n`
        txt += `> .autoai on --Panzzmode=kobo --type=voice`
        return m.reply(txt)
    }
