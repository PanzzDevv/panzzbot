...
const pluginConfig = {
    name: 'Panzzanime',
    alias: ['animagine', 'Panzz-anime', 'xl-anime'],
    category: 'ai',
    description: 'Generate anime dengan Animagine XL 4.0',
    usage: '.Panzzanime <karakter>',
    example: '.Panzzanime Shiroko',
    isOwner: false,
    isPremium: false,
    isGroup: false,
    isPrivate: false,
...
let thumbAnime = null
try {
    const p = path.join(process.cwd(), 'assets/images/Panzz-ai.jpg')
...
async function handler(m, { sock }) {
    ...
    if (!prompt) {
        return m.reply(
            ...
            `*ᴄᴀʀᴀ ᴘᴀᴋᴀɪ:*\n` +
            `> \`${m.prefix}Panzzanime <prompt> [ratio]\`\n\n` +
            `*ᴄᴏɴᴛᴏʜ:*\n` +
            `> \`${m.prefix}Panzzanime Shiroko\`\n` +
            `> \`${m.prefix}Panzzanime Raiden Shogun 16:9\`\n` +
            `> \`${m.prefix}Panzzanime cute girl, pink hair 9:16\`\n\n` +
            `*ʀᴀᴛɪᴏ:*\n` +
            `> ${VALID_RATIOS.join(', ')}\n\n` +
            `*ᴛɪᴘs:*\n` +
    ...
    try {
        ...
        const caption = `🌸 *ᴏᴜʀɪɴ ᴀɴɪᴍᴇ ɢᴇɴᴇʀᴀᴛᴇᴅ*\n\n` +
            ...
            `╰┈┈┈┈┈┈┈┈⬡\n\n` +
            `> ${config.bot?.name || 'Panzz-AI'}`
        
        await sock.sendMessage(m.chat, {
            image: { url: data.result },
            caption,
            contextInfo: getContextInfo('🌸 Panzz ANIME', prompt, thumbBuffer)
        }, { quoted: m })
